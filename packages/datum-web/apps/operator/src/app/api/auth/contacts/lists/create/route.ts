...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(SERVICE_APP_ROUTES.contactLists, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      await handleResponseError(response, 'Failed to create lists')
    }

    const data = await response.json()
    return NextResponse.json(data, { status: HttpStatus.Ok })
  } catch (error: any) {
    return handleError(error, 'Failed to create lists')
  }
}
