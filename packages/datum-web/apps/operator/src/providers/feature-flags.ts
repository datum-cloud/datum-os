...
export async function getFeatureFlagBool(flagKey: string, defaultValue: any) {
  return featureFlagClient.getBooleanValue(flagKey, defaultValue, await context())
}

export async function getFeatureFlagString(flagKey: string, defaultValue: any) {
  return featureFlagClient.getStringValue(flagKey, defaultValue, await context())
}

export async function getFeatureFlagNumber(flagKey: string, defaultValue: any) {
  return featureFlagClient.getNumberValue(flagKey, defaultValue, await context())
}

export async function getFeatureFlagObject(flagKey: string, defaultValue: any) {
  return featureFlagClient.getObjectValue(flagKey, defaultValue, await context())
}
